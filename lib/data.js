const DEFAULT_CSV_URL = 'https://docs.google.com/spreadsheets/d/1LJYK6OSqnsUz8DiL8YgpqsYkgAcJCwCfh0wp-C_iiwA/export?format=csv&gid=980830651';
const CSV_URL = process.env.CSV_URL || DEFAULT_CSV_URL;

const CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes

function now() { return Date.now(); }

async function fetchText(url) {
  const r = await fetch(url, {
    redirect: 'follow',
    headers: { 'user-agent': 'rental-king/1.0 (+vercel)' }
  });
  if (!r.ok) throw new Error('CSV fetch failed: ' + r.status);
  return await r.text();
}

function parseCSV(text){
  const rows = []; let row=[], cur='', inQ=false;
  for (let i=0;i<text.length;i++){
    const c=text[i], n=text[i+1];
    if (inQ){
      if (c==='"' && n==='"'){ cur+='"'; i++; }
      else if (c==='"'){ inQ=false; }
      else cur+=c;
    } else {
      if (c==='"') inQ=true;
      else if (c===','){ row.push(cur); cur=''; }
      else if (c==='\n'){ row.push(cur); rows.push(row); row=[]; cur=''; }
      else if (c!=='\r'){ cur+=c; }
    }
  }
  if (cur.length || row.length){ row.push(cur); rows.push(row); }
  return rows;
}

function indexHeaders(h) { const m = {}; h.forEach((x,i)=>{ m[(x||'').trim()] = i; }); return m; }
function get(r, idx, k){ const i = idx[k]; return i==null ? '' : (r[i] ?? ''); }
function toNum(v){ const n = Number(String(v||'').trim()); return Number.isFinite(n) ? n : NaN; }
function genMap(a,c){ if(!a) return ''; const q=[a,c].filter(Boolean).join(' '); return 'https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(q); }
function normDate(v){ const s=String(v||'').trim(); if(!s) return ''; const d=new Date(s); return isNaN(d)?s:d.toISOString(); }

async function buildFromCSV() {
  if (!CSV_URL || !/^https?:\/\//.test(CSV_URL)) {
    throw new Error('CSV_URL is not set. Configure env var CSV_URL.');
  }
  const text = await fetchText(CSV_URL);
  const rows = parseCSV(text);

  const headers = rows.shift()?.map(h => (h || '').trim()) || [];
  const idx = indexHeaders(headers);

  function toImageCdnUrl(s){
    const v = String(s||'').trim();
    if (!v) return '';
    // Ignore Drive folder links
    if (/drive\.google\.com\/drive\/folders\//i.test(v)) return '';
    // Convert file view links to lh3 CDN
    // Pattern 1: https://drive.google.com/file/d/FILE_ID/view?usp=sharing
    const m1 = v.match(/drive\.google\.com\/file\/d\/([\w-]+)/i);
    if (m1) return `https://lh3.googleusercontent.com/d/${m1[1]}=w1600-no`;
    // Pattern 2: https://drive.google.com/uc?export=view&id=FILE_ID
    const m2 = v.match(/[?&]id=([\w-]+)/i);
    if (m2) return `https://lh3.googleusercontent.com/d/${m2[1]}=w1600-no`;
    return v;
  }

  const props = rows
    .filter(r => {
      const has = (k) => String(get(r, idx, k) || '').trim().length > 0;
      // Include rows that have at least one of these populated
      return has('id') || has('address') || has('title') || has('city');
    })
    .map(r => {
      const id    = get(r, idx, 'id');
      const addr  = get(r, idx, 'address');
      const city  = get(r, idx, 'city');
      const title = get(r, idx, 'title') || addr || id;
      const status = (get(r, idx, 'status') || '').toLowerCase();
      const latRaw = toNum(get(r, idx, 'lat'));
      const lngRaw = toNum(get(r, idx, 'lng'));
      const isValidCoord = (n) => Number.isFinite(n) && n !== 0;
      const lat = isValidCoord(latRaw) ? latRaw : null;
      const lng = isValidCoord(lngRaw) ? lngRaw : null;
      const photo = toImageCdnUrl(get(r, idx, 'primary_photo'));
      return {
        id,
        title,
        address: addr,
        city,
        type: get(r, idx, 'type'),
        beds: toNum(get(r, idx, 'beds')),
        baths: toNum(get(r, idx, 'baths')),
        rent: toNum(get(r, idx, 'rent')),
        sqft: get(r, idx, 'sqft'),
        status,
        map_link: get(r, idx, 'map_link') || genMap(addr, city),
        parking: get(r, idx, 'parking'),
        lease_start: normDate(get(r, idx, 'lease_start')),
        lat,
        lng,
        primary_photo: photo
      };
    });

  return { ok: true, properties: props };
}

function getCache() {
  if (!global.__rentalKingCache) global.__rentalKingCache = { ts: 0, data: null, meta: null };
  return global.__rentalKingCache;
}

async function getData(options) {
  const fresh = !!(options && options.fresh);
  const cache = getCache();
  const cacheFresh = cache.data && (now() - cache.ts) < CACHE_TTL_MS;
  if (!fresh && cacheFresh) return cache.data;
  const data = await buildFromCSV();
  cache.data = data;
  cache.meta = { updatedAt: new Date().toISOString(), source: 'csv', url: CSV_URL };
  cache.ts = now();
  return data;
}

function getMeta() {
  const cache = getCache();
  return cache.meta;
}

module.exports = { getData, getMeta };

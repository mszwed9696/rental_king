...
csvLines.forEach((line, index) => {
  ...
  const latLngStr = columns[15]?.trim(); // This should now have "lat, lng"
...
data.properties.forEach((property, index) => {
  if (coordinateMap[property.id]) {
    ...
    if (index < 10) {
      console.log(`${(index + 1).toString().padStart(3)}. ${property.id.padEnd(15)} | ${property.address.padEnd(30)} | ${property.lat.toFixed(6)}, ${property.lng.toFixed(6)}`);
    }
  } else {
    notFoundCount++;
    if (notFoundCount <= 5) {
...
console.log(`\nMap pins should now appear for all properties with coordinates!`);
